...
#define ERROR_ENTRY_NODEF(id) ERROR_ENTRY(-id, "Ќеопределенна€ ошибка") //1 неопределенный элемент ошибок
...
namespace Error
{
	struct ERROR
	{
		...
		struct IN//расширение дл€ ошибок при обработке данных
		{
	...
	ERROR geterror(int id); //сформировать ERROR дл€ ERROR_THROW 
	ERROR geterrorin(int id, int line, int col);//сформировать ERROR дл€ ERROR_THROW_IN
}
