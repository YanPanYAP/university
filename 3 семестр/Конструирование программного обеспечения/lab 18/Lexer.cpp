...
namespace LA
{
	// ‘ункци€ дл€ поиска и анализа лексем в исходном тексте
	void FindLex(const In::IN& source, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		char* temp = new char[token_size] {};	// —оздаем временный массив дл€ хранени€ токенов
		int str_number = 1;
		int str_position = 1;
		In::IN in;
		...
		{
			// ѕроверка символов, которые не €вл€ютс€ разделител€ми или операторами
			...
			{
				// ≈сли накоплен токен, анализируем его
				if (j != 0)
				{
					i--;	// —двиг позиции назад, чтобы повторно обработать текущий символ
					temp[j] = '\0';
					if (Analyse(temp, str_number, lexTable, idTable))	// ≈сли токен успешно проанализирован
					{
						temp[0] = '\0'; j = 0;
						continue;
					}
				...
				{
					// ќбработка строковых литералов, заключенных в одинарные кавычки
					if (source.text[i] == '\'')
					{
						temp[j++] = source.text[i++];
			...
			// ≈сли найден символ новой строки '|', обновл€ем номер строки
			if (source.text[i] == '|')
			{
				str_number++;
				str_position = 0;
	...
	// ‘ункци€ дл€ анализа и классификации токенов
	bool Analyse(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		static TypeOfVar flag_type_variable;	// ѕеременна€ дл€ отслеживани€ типа переменной
		...
		// ќбработка строкового литерала, заключенного в одинарные кавычки
		case '\'':
		{
		...
		// ќбработка числового литерала и идентификаторов
		default:
		{
			...
			if (FST::execute(*number_literal))
			{
				...
				else
				{
					...
					idTable.table[idTable.size - 1].value.vint = atoi(token);	// ѕреобразуем строку токена в целое число и сохран€ем его значение
					Add(lexTable, { LEX_LITERAL,str_number, idTable.size - 1 });
				}
			...
			else
			{
				...
				// ѕереходим к обработке идентификатора, если это не числовой литерал
				return AnalyseIdentificator(token, str_number, lexTable, idTable, flag_type_variable);
			}
		}
	...
	// ‘ункци€ дл€ анализа идентификаторов и проверки их области видимости
	bool AnalyseIdentificator(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable, TypeOfVar& flag_type_variable)
	{
		...
		if (FST::execute(*id))
		{
			bool Checked_id = false;	// ‘лаг, указывающий, был ли идентификатор проверен
					...
					flag_type_variable.LT_posititon == lexTable.size - 2))
			{
				if (strcmp(token, "main") == 0)
				{
					for (int iter = 0; iter < idTable.size; iter++)
					{
						...
						{
							throw ERROR_THROW_IN(129, str_number, -1);	// ќшибка при дублировании имени функции
						}
					}
				}
			...
			//»дентификаторы с учЄтом области видимости
			if (!Checked_id)
			{
				bool LeftBrace = false;
				for (int i = lexTable.size - 1; i >= 0; i--)
				{
						...
						idTable.GetEntry(lexTable.GetEntry(i).idxTI).idtype == IT::IDTYPE::F)
					{
						...
						{
							temp = idTable.IsId(token);		// ѕровер€ем идентификатор без области видимости
