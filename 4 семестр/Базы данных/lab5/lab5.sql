...
VALUES ('Новый Заказчик', 'Минск, ул.Новая, 1', 'BY30UNBS30130');


/*LAB 4*/

-- 1. Список заказов с наименованием заказчика
...
-- 1. Список контактных лиц заказчиков с их фирмами
...
-- 1. Категоризация товаров по ценам
...
-- 2. Определение скидки на товар в зависимости от цены
SELECT Наименование, Цена,
       CASE 
           WHEN Цена > 5000 THEN Цена * 0.9  -- 10% скидка
           WHEN Цена BETWEEN 2000 AND 5000 THEN Цена * 0.95 -- 5% скидка
           ELSE Цена -- без скидки
       END AS Цена_со_скидкой
FROM ТОВАРЫ;

-- 3. Оценка прибыльности заказов (если цена продажи выше закупочной)
...
-- 1. Перебор всех возможных комбинаций товаров и заказчиков
SELECT ТОВАРЫ.Наименование, ЗАКАЗЧИКИ.Наименование_фирмы
FROM ТОВАРЫ
CROSS JOIN ЗАКАЗЧИКИ;

-- 2. Список всех возможных пар заказов и товаров
SELECT ЗАКАЗЫ.Номер_заказа, ТОВАРЫ.Наименование
FROM ЗАКАЗЫ
CROSS JOIN ТОВАРЫ;

-- 3. Все возможные комбинации контактных лиц и товаров
SELECT КОНТАКТЫ_ЗАКАЗЧИКОВ.Контактное_лицо, ТОВАРЫ.Наименование
FROM КОНТАКТЫ_ЗАКАЗЧИКОВ
CROSS JOIN ТОВАРЫ;
...
FROM ТОВАРЫ
INNER JOIN ДЕТАЛИ_ЗАКАЗА ON ТОВАРЫ.id = ДЕТАЛИ_ЗАКАЗА.товар_id
INNER JOIN ЗАКАЗЫ ON ДЕТАЛИ_ЗАКАЗА.заказ_id = ЗАКАЗЫ.id
INNER JOIN ЗАКАЗЧИКИ ON ЗАКАЗЫ.заказчик_id = ЗАКАЗЧИКИ.id
WHERE ЗАКАЗЧИКИ.Адрес LIKE '%Минск%';
...
SELECT ЗАКАЗЧИКИ.Наименование_фирмы, ТОВАРЫ.Наименование, ТОВАРЫ.Цена
FROM ТОВАРЫ
INNER JOIN ДЕТАЛИ_ЗАКАЗА ON ТОВАРЫ.id = ДЕТАЛИ_ЗАКАЗА.товар_id
INNER JOIN ЗАКАЗЫ ON ДЕТАЛИ_ЗАКАЗА.заказ_id = ЗАКАЗЫ.id
INNER JOIN ЗАКАЗЧИКИ ON ЗАКАЗЫ.заказчик_id = ЗАКАЗЧИКИ.id
WHERE ТОВАРЫ.Цена = (
    ...
    WHERE id IN (
        ...
        WHERE заказ_id IN (
            SELECT id
            FROM ЗАКАЗЫ
            WHERE заказчик_id = ЗАКАЗЧИКИ.id
        )
    )
);
...
WHERE NOT EXISTS (
    SELECT 1
    FROM ЗАКАЗЫ
    WHERE ЗАКАЗЫ.заказчик_id = ЗАКАЗЧИКИ.id
);

SELECT 
    (SELECT AVG(Цена_продажи) FROM ДЕТАЛИ_ЗАКАЗА WHERE заказ_id IN (SELECT id FROM ЗАКАЗЫ WHERE Дата_оформления BETWEEN '2025-03-01' AND '2025-03-31')) AS Средняя_цена_март
FROM dual;
...
WHERE Цена > ALL (
    SELECT Цена
    FROM ТОВАРЫ
    WHERE Наименование LIKE '%Ноутбук%'
);
...
WHERE Цена > ANY (
    SELECT Цена
    FROM ТОВАРЫ
    WHERE Наименование LIKE '%Смартфон%'
);
