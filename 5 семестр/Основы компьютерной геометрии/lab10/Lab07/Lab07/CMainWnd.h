...
public:
	CMainWnd() {
		Create(NULL, _T("Lab10"), WS_OVERLAPPEDWINDOW, CRect(0, 0, 600, 600), NULL, NULL);
		Index = 0;
		PView.RedimMatrix(3);
...
BEGIN_MESSAGE_MAP(CMainWnd, CFrameWnd)
	...
	ON_COMMAND(ID_APP_LIGHT, OnAppLight)
...
void CMainWnd::OnPaint() {
	CPaintDC dc(this);
	CString ss;
	ss.Format(_T("Наблюдатель: r=%.1f, fi=%.1f, th=%.1f"), PView(0), PView(1), PView(2));
	dc.TextOutW(0, 0, ss);
	ss.Format(_T("Свет: r=%.1f, fi=%.1f, th=%.1f"), PView2(0), PView2(1), PView2(2));
	dc.TextOutW(0, 50, ss);
	if (Index == 1)	///диф
...
void CMainWnd::OnAppLight() {
	CCoordDialog dlg;
	dlg.m_pPoint = &PView2;  // Источник света
	dlg.m_bIsCamera = false;

	if (dlg.DoModal() == IDOK) {
		// Значения уже установлены в диалоге
		Invalidate();  // Перерисовать окно
	}
}

void CMainWnd::OnAppCamera() {
	CCoordDialog dlg;
	dlg.m_pPoint = &PView;  // Камера
	dlg.m_bIsCamera = true;
