...
void CPlot3D::Draw(CDC& dc)
{
	...
	CPoint pt[4]; // массив из 4 точек — вершины одного полигона
	int kRegion = GetNumberRegion(); // определяет, с какой стороны наблюдатель смотрит на поверхность
	int nRows = MatrWindow.size(); // количество строк (по Y)
	int nCols = MatrWindow[0].size(); // количество столбцов (по X)
	switch (kRegion)
	{//
	case 1: // Region 1 — наблюдатель снизу слева
	{ // Обход по столбцам справа налево и по строкам сверху вниз
	...
	case 2: // Region 2 — наблюдатель сверху слева
	{ // Обход по строкам сверху вниз и по столбцам слева направо
		for (int i = 0; i < nRows - 1; i++)
	...
	case 3: // Region 3 — наблюдатель сверху справа
	{ // Обход по столбцам слева направо и по строкам сверху вниз
		for (int j = 0; j < nCols - 1; j++)
			for (int i = 0; i < nRows - 1; i++)
			{
				pt[0] = MatrWindow[i][j];
	...
	case 4: // Region 4 — наблюдатель снизу справа
	{ // Обход по строкам снизу вверх и по столбцам слева направо
