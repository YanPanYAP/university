function buildHistogramSvg(title, stats) {
  ...
  const bars = stats.frequencies
    .map((item, index) => {
      ...
      return `
        ...
        <text x="${centerX.toFixed(2)}" y="${labelY.toFixed(2)}" class="value">${item.frequency.toFixed(3)}</text>
        <text x="${centerX.toFixed(2)}" y="${(margin.top + chartHeight + 35).toFixed(2)}" class="symbol">${item.symbol}</text>
      `;
    })
    .join("");
  ...
  const horizontalGrid = Array.from({ length: gridSteps + 1 }, (_, index) => {
    ...
    return `
      ...
      <text x="${margin.left - 12}" y="${(y + 4).toFixed(2)}" class="axis">${value.toFixed(2)}</text>
    `;
  }).join("");
